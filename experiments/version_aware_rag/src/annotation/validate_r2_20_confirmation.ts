import {
  R220ConfirmationAnnotation,
  R220ConfirmationAnnotationSchema,
} from "./r2_20_confirmation_schema";

export const R220_CONFIRMATION_MINIMUM_COUNTS = {
...
export interface R220ConfirmationValidationOptions {
...
export interface R220ConfirmationValidationError {
  type: string;
  id: string;
  message: string;
}
...
export function r220EvidenceKey(evidence: {
...
export function validateR220ConfirmationLedger(
  rawRecords: unknown[],
  options: R220ConfirmationValidationOptions = {},
): R220ConfirmationValidationError[] {
  const errors: R220ConfirmationValidationError[] = [];
  const records: R220ConfirmationAnnotation[] = [];
  for (const [index, raw] of rawRecords.entries()) {
    const parsed = R220ConfirmationAnnotationSchema.safeParse(raw);
    if (!parsed.success) {
      errors.push({
        type: "Schema",
        id:
  ...
  const counts: Record<keyof typeof R220_CONFIRMATION_MINIMUM_COUNTS, number> = {
    conditional_merge: 0,
  ...
  for (const record of records) {
    ...
    const unsafe = new Set(
      [...record.deprecated_evidence, ...record.forbidden_evidence].map(r220EvidenceKey),
    );
    ...
    for (const evidence of required) {
      const key = r220EvidenceKey(evidence);
      if (unsafe.has(key)) {
        errors.push({
  ...
  if (options.requireFreezeReady) {
    for (const [stratum, minimum] of Object.entries(R220_CONFIRMATION_MINIMUM_COUNTS)) {
