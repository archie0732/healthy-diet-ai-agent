import {
  R212ConfirmationAnnotation,
  R212ConfirmationAnnotationSchema,
} from "./r2_12_confirmation_schema";

export interface R212ConfirmationValidationOptions {
...
export interface R212ConfirmationValidationError {
  type: string;
  id: string;
  message: string;
}

export const R212_CONFIRMATION_MINIMUM_COUNTS = {
...
export function r212EvidenceKey(evidence: {
...
export function validateR212ConfirmationLedger(
  rawRecords: unknown[],
  options: R212ConfirmationValidationOptions = {},
): R212ConfirmationValidationError[] {
  const errors: R212ConfirmationValidationError[] = [];
  const records: R212ConfirmationAnnotation[] = [];
  for (const [index, raw] of rawRecords.entries()) {
    const parsed = R212ConfirmationAnnotationSchema.safeParse(raw);
    if (!parsed.success) {
      errors.push({
        type: "Schema",
        id:
  ...
  const counts: Record<keyof typeof R212_CONFIRMATION_MINIMUM_COUNTS, number> = {
    conditional_merge: 0,
  ...
  for (const record of records) {
    ...
    if (queryIds.has(record.query_id)) {
      errors.push({
        type: "DuplicateQuery",
        id: record.query_id,
        message: "query_id must be unique.",
      });
    }
    ...
    if (lineageIds.has(record.lineage_group_id)) {
      errors.push({
        ...
        message: "lineage_group_id must be unique.",
      });
    }
    ...
    if (
      ...
      record.candidate_group_manifest_sha256 !==
        options.candidateGroupManifestSha256
    ) {
      errors.push({
    ...
    if (needsRetained && EXPLICIT_HISTORY_CUE.test(record.query_text)) {
      errors.push({
        type: "ImplicitIntent",
        id: record.query_id,
    ...
    if (!needsRetained && record.required_retained_evidence.length > 0) {
      errors.push({
        type: "StratumContract",
        id: record.query_id,
    ...
    const unsafe = new Set(
      [...record.deprecated_evidence, ...record.forbidden_evidence].map(
        r212EvidenceKey,
      ),
    );
    ...
    for (const evidence of required) {
      const key = r212EvidenceKey(evidence);
      if (unsafe.has(key)) {
        errors.push({
