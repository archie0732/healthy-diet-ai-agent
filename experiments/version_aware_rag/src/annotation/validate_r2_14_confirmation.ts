import {
  R214ConfirmationAnnotation,
  R214ConfirmationAnnotationSchema,
} from "./r2_14_confirmation_schema";

export const R214_CONFIRMATION_MINIMUM_COUNTS = {
...
export interface R214ConfirmationValidationOptions {
...
export interface R214ConfirmationValidationError {
  type: string;
  id: string;
  message: string;
}
...
export function r214EvidenceKey(evidence: {
...
export function validateR214ConfirmationLedger(
  rawRecords: unknown[],
  options: R214ConfirmationValidationOptions = {},
): R214ConfirmationValidationError[] {
  const errors: R214ConfirmationValidationError[] = [];
  const records: R214ConfirmationAnnotation[] = [];
  for (const [index, raw] of rawRecords.entries()) {
    const parsed = R214ConfirmationAnnotationSchema.safeParse(raw);
    if (!parsed.success) {
      errors.push({
        type: "Schema",
        id:
  ...
  const counts: Record<keyof typeof R214_CONFIRMATION_MINIMUM_COUNTS, number> = {
    conditional_merge: 0,
  ...
  for (const record of records) {
    ...
    const unsafe = new Set(
      [...record.deprecated_evidence, ...record.forbidden_evidence].map(r214EvidenceKey),
    );
    ...
    for (const evidence of required) {
      const key = r214EvidenceKey(evidence);
      if (unsafe.has(key)) {
        errors.push({
  ...
  if (options.requireFreezeReady) {
    for (const [stratum, minimum] of Object.entries(R214_CONFIRMATION_MINIMUM_COUNTS)) {
