import { RelationDetector, RelationDetectorOutput } from './types';
...
export class RuleBaselineDetector implements RelationDetector {
  public async classify(input: {
    ...
    // Extract numeric claims first to avoid mistaking numerical updates for exact duplicates
    ...
    const diffNumbers = (oldNumbers.length > 0 && newNumbers.length > 0) &&
      (oldNumbers.some(n => !newNumbers.includes(n)) || newNumbers.some(n => !oldNumbers.includes(n)));

    if (diffNumbers) {
      return {
        relationType: 'superseded',
        confidence: 0.8,
        rationale: `Numerical claims modified between versions: [${oldNumbers.join(', ')}] -> [${newNumbers.join(', ')}].`,
    ...
    if (jaccard > 0.9 || oldText === newText) {
      return {
    ...
    const activeKeywords = ['active', 'athletes', 'sweat', 'highly active', 'exercise', 'strenuous'];
    ...
    if (newHasActive !== oldHasActive) {
      return {
        ...
        confidence: 0.85,
        rationale: 'Guideline introduces conditional population terms not present in the other version.',
        modelInfo: { detector: 'rule_baseline' },
        latencyMs: Date.now() - startTime
      };
    }
