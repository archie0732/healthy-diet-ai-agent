...
describe("V5 R2.18 candidate failure-attribution diagnostic", () => {
  test("locks with no eligible lexical repair", () => {
    const config = path.join(
      BASE,
      "data/configs/v5_r2_18_candidate_failure_attribution",
    );
    const out = path.join(
      BASE,
      "results/v5/r2_18_candidate_failure_attribution",
    );
    ...
    ).toEqual([44 / 52, 44 / 52, 43 / 52, 43 / 52]);
    expect(result.r2_16_rerun_performed).toBe(false);
    expect(audit.iterative_closure_changes_vs_control).toHaveLength(2);
