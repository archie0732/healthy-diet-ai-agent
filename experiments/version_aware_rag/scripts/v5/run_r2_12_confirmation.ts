...
function bm25(query: string, corpus: Item[], k1: number, b: number) {
  ...
  const averageLength =
    lengths.reduce((sum, length) => sum + length, 0) / Math.max(1, lengths.length);
...
function rerank(
  ...
  const clauseFactor =
    1 +
    Math.min(3, (query.match(/\band\b|,|;|\bwhile\b|\bwhich\b/gi) ?? []).length) *
      0.2;
  return normalized
    .map((entry) => {
      let pairSignal = 0;
      ...
      return {
        item: entry.item,
        score: entry.base + recencyWeight * entry.recency + groupBoost * pairSignal,
        base: entry.base,
...
if (
  ...
  throw new Error("R2.12 confirmation guard failed.");
}
const queries = parseJsonl(queriesText) as Query[];
const corpus = parseJsonl(corpusText) as Item[];
const variants = ["whole_query_bm25", "bm25_group_expand_seed14"];
const raw: any[] = [];
for (const query of queries) {
  ...
  const repairedIds = generateR212CandidatePool(
    query.text,
    corpus.map((item) => ({
  ...
  const pools: Record<string, Item[]> = {
    whole_query_bm25: ranked
      .slice(0, manifest.parameters.pool_size)
      .map((entry) => entry.item),
    bm25_group_expand_seed14: repairedIds.map((id) => itemById.get(id)!),
  };
  for (const variant of variants) {
    const reranked = rerank(
      ...
      pools[variant],
      scoreById,
      manifest.parameters.downstream_recency_weight,
      manifest.parameters.downstream_group_boost,
    );
    const poolIds = pools[variant].map((item) => item.runtime_item_id);
    raw.push({
      runtime_query_key: query.runtime_query_key,
      variant,
      ordered_top20_ids: poolIds,
      ordered_top20_sha256: sha256(poolIds.join("\n")),
      top3: reranked.slice(0, 3).map((entry) => entry.item.runtime_item_id),
      full_scores: reranked.map((entry) => ({
        runtime_item_id: entry.item.runtime_item_id,
        score: entry.score,
        base_norm: entry.base,
        recency_norm: entry.recency,
        pair_signal: entry.pair_signal,
      })),
    });
  }
}
...
const evaluated = raw.map((row) => {
  ...
  const poolHits = row.ordered_top20_ids.filter((id: string) => required.has(id)).length;
  ...
  return {
    ...
    both_evidence_coverage: required.size === 2 ? Number(topHits === 2) : null,
...
const gateChecks = {
  ...
  implicit_both_evidence_coverage_strictly_improves:
    mergeBoth(repaired) > mergeBoth(baseline),
  ...
  unsafe_top3_hit_rate_not_increased:
    unsafeRate("bm25_group_expand_seed14") <= unsafeRate("whole_query_bm25"),
  ordered_pool_hash_integrity_100_percent: evaluated.every(
    (row) => sha256(row.ordered_top20_ids.join("\n")) === row.ordered_top20_sha256,
  ),
};
const passed = Object.values(gateChecks).every(Boolean);
const result = {
  schema_version: "v5-r2.12-confirmation-result-1",
  status: passed
  ...
  combined_merge_both_evidence_coverage: {
    baseline: mergeBoth(baseline),
    repaired: mergeBoth(repaired),
  },
  overall_unsafe_top3_hit_rate: {
    baseline: unsafeRate("whole_query_bm25"),
    repaired: unsafeRate("bm25_group_expand_seed14"),
  },
  gate_checks: gateChecks,
  gate_passed: passed,
  claim_boundary: passed
    ? "Scope-limited Development confirmation only; Validation remains separately gated."
    : "No promotion; R2.12 remains Development-only.",
};
...
await writeFile(
  ...
  `${JSON.stringify(
    {
      ...guard,
      status: passed ? "confirmation_passed_locked" : "confirmation_failed_locked",
      retrieval_execution_count: 1,
