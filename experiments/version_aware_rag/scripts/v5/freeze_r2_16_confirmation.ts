...
const OUT = path.join(EXP, "data/configs/v5_r2_16_frozen_confirmation");
...
const [
  ...
  readFile(path.join(EXP, "R2_16_CONFIRMATION_OWNER_REVIEW_PACKET.md"), "utf8"),
  ...
  readFile(path.join(EXP, "scripts/v5/run_r2_16_confirmation.ts"), "utf8"),
  readFile(path.join(EXP, "R2_16_TOP6_ANCHORED_CONFIRMATION_PROTOCOL.md"), "utf8"),
...
if (
  ...
  throw new Error("R2.16 owner-approval checksum boundary failed.");
}
  ...
  for (const record of parseJsonl(text)) {
    ...
    for (const field of [
      ...
      for (const item of record[field] ?? []) {
        forbiddenEvidence.add(r216EvidenceKey(item));
      }
    }
  }
}
...
const errors = validateR216ConfirmationLedger(approved, {
...
for (const group of groups) {
  for (const item of group.candidate_items) {
    const runtimeId = `r2.16-item-${sha256(item.evidence_key).slice(0, 18)}`;
...
for (const record of approved) {
  const queryKey = `r2.16-query-${sha256(record.query_id).slice(0, 16)}`;
  const toId = (item: any) =>
    runtimeItems.get(r216EvidenceKey(item)).runtime_item_id;
...
const signoff = {
  schema_version: "v5-r2.16-confirmation-owner-signoff-1",
...
const manifest = {
  schema_version: "v5-r2.16-frozen-confirmation-manifest-1",
...
const guard = {
  schema_version: "v5-r2.16-confirmation-guard-1",
  status: "confirmation_frozen_unlocked",
