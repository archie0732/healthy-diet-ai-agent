...
const OUT = path.join(EXP, "data/configs/v5_r2_14_frozen_confirmation");
...
const EXPECTED_PACKET =
  "0510bb955233517c873bbe9671dd8fff73412c1729ca08427f1970214f896a26";
const EXPECTED_LEDGER =
  "77b121be6a761b495631ce8972846691416079134e1cbc021021a5c0f40fabbf";
...
const [
  ...
  readFile(path.join(EXP, "R2_14_CONFIRMATION_OWNER_REVIEW_PACKET.md"), "utf8"),
  ...
  readFile(path.join(EXP, "scripts/v5/run_r2_14_confirmation.ts"), "utf8"),
  readFile(path.join(EXP, "R2_14_GATE_FEASIBLE_TOP3_CONFIRMATION_PROTOCOL.md"), "utf8"),
...
if (
  ...
  throw new Error("R2.14 owner-approval checksum boundary failed.");
}
...
for (const text of exclusionTexts) {
  for (const record of parseJsonl(text)) {
    if (record.lineage_group_id) {
      forbiddenLineages.add(record.lineage_group_id);
    }
    for (const field of [
      ...
      for (const item of record[field] ?? []) {
        forbiddenEvidence.add(r214EvidenceKey(item));
      }
    }
  }
}
...
const errors = validateR214ConfirmationLedger(approved, {
...
for (const group of groups) {
  for (const item of group.candidate_items) {
    const runtimeId = `r2.14-item-${sha256(item.evidence_key).slice(0, 18)}`;
...
for (const record of approved) {
  const queryKey = `r2.14-query-${sha256(record.query_id).slice(0, 16)}`;
  const toId = (item: any) =>
    runtimeItems.get(r214EvidenceKey(item)).runtime_item_id;
...
const signoff = {
  schema_version: "v5-r2.14-confirmation-owner-signoff-1",
...
const manifest = {
  schema_version: "v5-r2.14-frozen-confirmation-manifest-1",
...
const guard = {
  schema_version: "v5-r2.14-confirmation-guard-1",
  status: "confirmation_frozen_unlocked",
