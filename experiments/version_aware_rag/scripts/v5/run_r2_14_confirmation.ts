...
if (
  ...
  throw new Error("R2.14 confirmation guard failed.");
}
const queries = parseJsonl(queriesText) as Query[];
const corpus = parseJsonl(corpusText) as Item[];
const variants = ["pair_score_g0.5", "pair_score_g2.0"] as const;
const raw: any[] = [];
...
const hardConstraints = {
  ...
  unsafe_top3_hit_rate_not_increased:
    unsafeRate("pair_score_g2.0") <= unsafeRate("pair_score_g0.5"),
...
const result = {
  ...
  overall_unsafe_top3_hit_rate: {
    ...
    repaired: unsafeRate("pair_score_g2.0"),
  },
  ...
  claim_boundary: passed
    ...
    : "No promotion; R2.14 remains Development-only.",
};
