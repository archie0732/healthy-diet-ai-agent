...
import {
  R216_CONFIRMATION_MINIMUM_COUNTS,
  r216EvidenceKey,
  validateR216ConfirmationLedger,
} from "../../src/annotation/validate_r2_16_confirmation";
import { R216_ANNOTATION_SPECS } from "./r2_16_annotation_specs";

const EXP = path.join(process.cwd(), "experiments/version_aware_rag");
const GROUP_DIR = path.join(EXP, "data/annotations_v5/r2_16_predeclared_candidate_groups");
const REVIEW_DIR = path.join(EXP, "data/annotations_v5/r2_16_candidate_groups_codex_reviewed");
const PLAN_DIR = path.join(EXP, "data/annotations_v5/r2_16_annotation_authoring_plan");
const OUT = path.join(EXP, "data/annotations_v5/r2_16_confirmation_codex_reviewed");
const PACKET = path.join(EXP, "R2_16_CONFIRMATION_OWNER_REVIEW_PACKET.md");
...
const evidence = (
  ...
  return {
    item_id: `r2.16-${specId}-${role}`,
    role,
    document_id: candidate.document_id,
    atomic_claim_text: claim,
    official_record_url: document.official_record_url ?? document.official_page_url,
...
const records = R216_ANNOTATION_SPECS.map((spec) => {
  ...
  return {
    schema_version: "v5-r2.16-confirmation-annotation-1",
    query_id: `r2.16-confirm-${spec.id}`,
    split: "development_confirmation",
    stratum: spec.stratum,
    lineage_group_id: `r2.16-lineage-${sha256(spec.group).slice(0, 18)}`,
    ...
    implicit_retained_rationale: implicit
      ? "The query requires two distinct clauses: the retained passage supplies still-applicable evidence or context and the current passage supplies present evidence, scope, safety, or implementation detail."
      : undefined,
    annotation_rationale:
      spec.stratum === "hard_negative_current"
        ...
        : "Evidence roles were assigned after candidate-group and stratum freeze, before any R2.16 retrieval execution.",
    review: {
...
const exclusionTexts = await Promise.all(
  exclusionPaths.map((relativePath) => readFile(path.join(EXP, relativePath), "utf8")),
);
  ...
  for (const record of parseJsonl(text)) {
    ...
    for (const field of ["required_current_evidence", "required_retained_evidence"]) {
      for (const item of record[field] ?? []) {
        forbiddenRequiredEvidence.add(r216EvidenceKey(item));
      }
    }
  }
}
...
const errors = validateR216ConfirmationLedger(records, options);
if (errors.length) {
  throw new Error(`R2.16 provisional ledger invalid: ${JSON.stringify(errors)}`);
}
const freezeErrors = validateR216ConfirmationLedger(records, {
  ...options,
...
const counts = Object.fromEntries(
  Object.keys(R216_CONFIRMATION_MINIMUM_COUNTS).map((stratum) => [
    stratum,
...
const packetText = `${[
  "# R2.16 Confirmation Owner Review Packet",
  "",
  "Scope: 32 new lineage-disjoint Development-confirmation annotations. No R2.16 retrieval has run.",
  "",
  ...
  "## Approval boundary",
  "",
  "Approval confirms only these 32 checksum-bound annotations. It authorizes freezing the R2.16 Development confirmation package and its one preregistered execution; it does not authorize Validation, fresh-test execution, or promotion.",
  "",
  "Exact bulk-approval phrase: `核准全部 32 筆`",
  "",
].join("\n")}\n`;
const manifest = {
  schema_version: "v5-r2.16-confirmation-provisional-manifest-1",
  status: "codex_provisional_owner_review_required",
  provisional_record_count: records.length,
  stratum_counts: counts,
  minimum_counts_satisfied: Object.entries(R216_CONFIRMATION_MINIMUM_COUNTS).every(
    ([stratum, minimum]) => counts[stratum] >= minimum,
  ),
  ...
  required_evidence_overlap_count: records
    ...
    .filter((item) => forbiddenRequiredEvidence.has(r216EvidenceKey(item))).length,
  candidate_group_manifest_sha256: groupManifestSha,
